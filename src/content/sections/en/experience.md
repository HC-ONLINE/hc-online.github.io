---
title: "Background & Education"
subtitle: "My academic path and technical experience"
educationTitle: "Academic Education"
experience:
  - period: "2022 - Present"
    role: "Backend & Security Developer (Independent)"
    company: "HC-ONLINE"
    description: "Design and development of web auditing tools, Identity and Access Management (IAM) systems, and LLM orchestration engines."
    achievements:
      - "Developed the CiberWebScan auditing engine"
      - "Implemented IAM Suite (AccessManager/PermissionManager)"
      - "Secure deployments using Docker and CI/CD pipelines"
    current: true
education:
  - degree: "Systems Engineering"
    institution: "Fundación Universitaria Tecnológico Comfenalco"
    year: "2026 - Present"
  - degree: "Network Management & Information Security Technology"
    institution: "Fundación Universitaria Tecnológico Comfenalco"
    year: "2022 - 2025 (Completed)"
  - degree: "Introduction to Cybersecurity"
    institution: "Cisco Networking Academy"
    year: "2024"
---

Experience and journey.
